...
import type { AnswerInput, StoredAnswer } from './types.js';
...
app.post('/api/respostas', (req, res) => {
  const { questionId, alternativaEscolhida } = req.body as Partial<AnswerInput>;
...
app.get('/api/resultados', (_req, res) => {
  const respostas = db.prepare('SELECT id, question_id AS questionId, alternativa_escolhida AS alternativaEscolhida, correta, timestamp FROM respostas ORDER BY id').all() as StoredAnswer[];
